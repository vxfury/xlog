...
void *fill_buffer( void *buf, size_t buf_size, const char *test_pattern )
{
	...
	size_t n;
	for(
	    n = 0; n != nblocks; ++n, p += pattern_size
	) {
...
int main( int argc, char **argv )
{
	...
	assert( ringbuf_tail( rb1 ) == rb1_base + 1 );
	assert(
	    strncmp( ringbuf_tail( rb1 ), ( const char * )buf + RINGBUF_SIZE, RINGBUF_SIZE - 1 ) == 0 );
	END_TEST( test_num );
	...
	ringbuf_reset( rb1 );
	assert( ringbuf_memcpy_into( rb1, buf, strlen( test_pattern ) ) == ringbuf_head( rb1 ) );
	assert( ringbuf_memcpy_into( rb1, buf + strlen( test_pattern ), strlen( test_pattern ) - 1 ) ==
	    ringbuf_head( rb1 ) );
	assert( ringbuf_capacity( rb1 ) == RINGBUF_SIZE - 1 );
	assert(
	    ringbuf_bytes_free( rb1 ) == ringbuf_capacity( rb1 ) - ( 2 * strlen( test_pattern ) - 1 ) );
	assert( !ringbuf_is_full( rb1 ) );
	assert( !ringbuf_is_empty( rb1 ) );
	assert(
	    strncmp( ( const char * )buf, ringbuf_tail( rb1 ), 2 * strlen( test_pattern ) - 1 ) == 0 );
	assert( *( char * )ringbuf_head( rb1 ) == '\1' );
	END_TEST( test_num );
	...
	assert( ringbuf_capacity( rb1 ) == RINGBUF_SIZE - 1 );
	assert(
	    ringbuf_bytes_free( rb1 ) == ringbuf_capacity( rb1 ) - ( strlen( test_pattern2 ) - 3 ) );
	assert( ringbuf_bytes_used( rb1 ) == strlen( test_pattern2 ) - 3 );
	assert( !ringbuf_is_full( rb1 ) );
	assert( !ringbuf_is_empty( rb1 ) );
	assert( ringbuf_tail( rb1 ) == rb1_base + 3 );
	assert(
	    ringbuf_head( rb1 ) == ( uint8_t * )ringbuf_tail( rb1 ) + ( strlen( test_pattern2 ) - 3 ) );
	assert( strncmp( ( const char * )dst, test_pattern2, 3 ) == 0 );
	assert(
	    strncmp( ( const char * )dst + 3, ( const char * )buf + 3, RINGBUF_SIZE * 2 - 3 ) == 0 );
	END_TEST( test_num );
	...
	assert( strncmp(
	        ( const char * )dst + RINGBUF_SIZE - 1, ( const char * )buf + RINGBUF_SIZE - 1,
	        RINGBUF_SIZE + 1
	    ) == 0 );
	END_TEST( test_num );
	...
	assert(
	    strncmp( ( const char * )dst + 13, ( const char * )buf + 13, RINGBUF_SIZE * 2 - 13 ) == 0 );
	END_TEST( test_num );
	...
	assert(
	    strncmp( ( const char * )dst + 14, ( const char * )buf + 14, RINGBUF_SIZE * 2 - 14 ) == 0 );
	END_TEST( test_num );
	...
	assert( strncmp( ( const char * )dst + 11, ( const char * )buf2 + 11, RINGBUF_SIZE * 2 - 11 ) ==
	    0 );
	assert( *( char * )ringbuf_head( rb1 ) == '\1' );
	END_TEST( test_num );
	...
	assert(
	    strncmp( ( const char * )dst + 7, ( const char * )buf2 + 7, RINGBUF_SIZE * 2 - 7 ) == 0 );
	...
	assert( strncmp(
	        ( const char * )dst + RINGBUF_SIZE - 1, ( const char * )buf2 + RINGBUF_SIZE - 1,
	        RINGBUF_SIZE * 2 - ( RINGBUF_SIZE - 1 )
	    ) == 0 );
	END_TEST( test_num );
	...
	assert( strncmp(
	        ( const char * )dst + RINGBUF_SIZE, ( const char * )buf2 + RINGBUF_SIZE, RINGBUF_SIZE
	    ) == 0 );
	END_TEST( test_num );
	...
	/* overflow */
	assert(
	    ringbuf_memcpy_into( rb1, buf + RINGBUF_SIZE, RINGBUF_SIZE - 1 ) == ringbuf_head( rb1 ) );
	assert( ringbuf_capacity( rb1 ) == RINGBUF_SIZE - 1 );
	assert( ringbuf_bytes_free( rb1 ) == 0 );
	assert( ringbuf_bytes_used( rb1 ) == RINGBUF_SIZE - 1 );
	assert( ringbuf_is_full( rb1 ) );
	...
	assert( strncmp(
	        ( const char * )ringbuf_tail( rb1 ) + RINGBUF_SIZE - 2, ( const char * )buf2, 1
	    ) == 0 );
	END_TEST( test_num );
	...
	assert( strncmp(
	        ( const char * )ringbuf_tail( rb1 ) + RINGBUF_SIZE - 3, ( const char * )buf2, 1
	    ) == 0 );
	...
	assert( strncmp(
	        ( const char * )ringbuf_tail( rb1 ) + 1, ( const char * )buf2 + RINGBUF_SIZE - 2, 3
	    ) == 0 );
	assert(
	    strncmp( ( const char * )rb1_base, ( const char * )buf2 + RINGBUF_SIZE - 2 + 3, 2 ) == 0 );
	END_TEST( test_num );
	...
	assert( strncmp(
	        ( const char * )ringbuf_tail( rb1 ) + RINGBUF_SIZE - 1 - 2, ( const char * )buf2, 1
	    ) == 0 );
	assert( strncmp( ( const char * )rb1_base, ( const char * )buf2 + 1, 1 ) == 0 );
	END_TEST( test_num );
